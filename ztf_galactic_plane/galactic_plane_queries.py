...
import numpy as np
from ztf_galactic_plane.constants import ztf_galactic_fields_list
...
def search_galactic_plane_candidates_from_fields(k, jd_start: float, jd_end: float,
                                     catalog: str='ZTF_alerts',
                                     filter_kwargs: dict = {},
                                     max_n_threads: int = 8):
    """Search for candidates in the galactic plane using Kowalski."""
    ...
    query = get_find_query(
        ...
        filter={
            "candidate.jd": {"$gt": jd_start, "$lt": jd_end},
            "candidate.field": {"$in": ztf_galactic_fields_list},
            "candidate.drb": {"$gt": 0.5},
            "candidate.ndethist": {"$gt": 5},
            "candidate.isdiffpos": {'$in': ['t', '1', True, 1]},
            "$or": [
